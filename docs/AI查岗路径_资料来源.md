...
### 11. 华南理工大学·就业指导中心（无需登录！）
```
网络招聘（109页）：https://jyzx.scut.edu.cn/37757/list.htm
校园宣讲：https://jyzx.scut.edu.cn/37755/list.htm
校外专场：https://jyzx.scut.edu.cn/37756/list.htm
搜索功能：企业名称/岗位名称输入框 + 时间筛选 → 可精确搜索
登录入口：job.xgb.scut.edu.cn（CAS 统一认证）
就业老师：本科 李文杰 020-81182711 / 研究生 王健 020-81182710
```
- 来源类型：✅ 详情页（无需登录，公司+岗位+日期公开显示）
- 数据格式：公司行 → 岗位行（| 分隔多个岗位）→ 日期行 (YYYY.MM.DD)
- 已接入 parser: `scut` 类型（parse_univ）
- 验证方式：browser-act 全浏览器逐页提取
- 可靠性：高（109页，每页~10条企业招聘信息）
- 限制：分页通过 JS 加载，目前只抓首页；全量需要点击分页按钮或CAS登录
